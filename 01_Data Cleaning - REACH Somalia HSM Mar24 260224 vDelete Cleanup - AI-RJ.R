...
source("_code/support_functions/01_Data Cleaning - Support Functions.R")
...
kobo_user_name <- "reidjackson"
kobo_pw <- "uqr9wbr-RVB_arx9gyx"
kobo_url <- "eu.kobotoolbox.org"
kobo_project_name <- "HSM March 2024 Data Collection Kobo Tool"
...
kobo_data_export_path <- paste0("_raw_data_download/hsm_mar24_raw_data_", date_time_now, ".xlsx")
...
data_in_processing <- raw_data %>%
  ...
  # first week of data
  # filter(as.Date(`_submission_time`) == '2024-03-03') %>%
  # filter(as.Date(`_submission_time`) == '2024-03-04') %>%
  # filter(as.Date(`_submission_time`) == '2024-03-05') %>%
  ...
  # third week of data
   #filter(as.Date(`_submission_time`) == '2024-03-17') %>%
    filter(as.Date(`_submission_time`) == '2024-03-18') %>%
...
fo_district_mapping <- read_excel("_FO_assignments_and_locations/fo_base_assignment_260224.xlsx") %>%
...
dashboard_data_path <- paste0("99_Data_Collection_Dashboard/_data_for_dashboard/unchecked_data_for_dashboard_", date_time_now, ".xlsx")
